...
# In this section we set project-wide options (working directory, caching folders, and timeouts).
# We keep these at the top so re-running the script is predictable across machines.
# =========================
...
# =========================
# 0.1) PACKAGES & SYSTEM
# We load the geospatial and data packages used throughout the paper (sf/terra/tmap/exactextractr).
# If a package is missing, the script installs it to reduce setup friction.
# =========================
pkgs <- c(
...
# =========================
# 0.2) PAPER PARAMETERS
# Here we define the baseline (central-case) parameters used in the paper and the sensitivity grids.
# We reference the PDF equation numbers in later sections.
# =========================
# Section 4.2–4.6 parameters (central case + sensitivity grids)
tau     <- 0.05     # Eq (1)
h0      <- 0.10     # Eq (6)-(11)
...
# We define helper functions that make the pipeline robust and reproducible:
#  - caching/downloading rasters,
#  - STAC JSON parsing,
#  - small utilities (safe defaults, formatting).
# =========================
...
# =========================
# 1) DATA: STUDY AREA (Section 3.1)
# We build the study area used in the paper: the Philippines plus a 20 km coastal belt.
# All layers are projected to EPSG:3857 (metres) so distance calculations (d(x)) are meaningful.
# =========================
...
# =========================
# 2) DATA: MANGROVES (Section 3.2)
# We download Global Mangrove Watch (GMW 2020), crop to the coastal belt, and resample to the flood grid.
# From the fractional cover we create a binary presence map (Eq. 1) and a distance-to-mangrove raster (Eq. 2).
# =========================
...
# =========================
# 3) DATA: FLOOD HAZARD (Section 3.3)
# We fetch Deltares coastal inundation depth maps via the Planetary Computer STAC API.
# Each return-period raster is cropped/masked to the coastal belt and cached locally as a GeoTIFF.
# =========================
...
# =========================
# 4) DATA: POPULATION (Section 3.4)
# We load WorldPop gridded population and align it to the flood template grid.
# This allows us to compute exposed population by multiplying (Pop * flooded_indicator) and summing by municipality.
# =========================
...
# =========================
# 5) DATA: NIGHT LIGHTS (Section 3.5)
# We load annual VIIRS night lights (2018) and align it to the flood template grid.
# Night lights are used both as an exposure proxy and to spatially allocate national GDP across pixels (Eq. 12).
# =========================
ntl_year <- 2018
...
# =========================
# 6) METHODOLOGY: MANGROVE COUNTERFACTUAL (Section 4.2, Eq 1–5)
# We implement the reduced-form mangrove attenuation model from the paper (Eq. 1–5).
# The key output is a set of flood-depth rasters 'with mangroves' vs 'no mangroves' for each return period.
# =========================
...
# Check: if cached file was created incorrectly earlier, it will fix automatically
...
# We translate distance-to-mangroves into a depth-reduction fraction r(x) (Eq. 3).
# The effect decays exponentially with distance and is set to zero beyond dmax.
...
# Given a flood-depth raster, we apply mangrove protection by multiplying depth by (1 - r(x)) (Eq. 4).
# We also resample the distance raster if needed so cell grids match before raster arithmetic.
...
# =========================
# 7) METHODOLOGY: EXPOSURE (Section 4.3, Eq 6–10)
# Using a depth threshold h0 (paper Eq. 6–10), we compute exposed population and exposed night lights by municipality.
# Avoided exposure is the difference between the no-mangrove baseline and the with-mangrove counterfactual.
# =========================
# For a given return period, we compute municipality-level exposure by summing values in flooded pixels only.
# We use exactextractr so partial pixel overlaps with municipal boundaries are handled correctly.
...
# =========================
# 8) METHODOLOGY: MONETARY VALUATION (Section 4.4, Eq 11–17)
# We convert hazard into money by: (i) allocating GDP via night lights, (ii) converting GDP to assets using K/Y,
# (iii) applying the Huizinga et al. (2017) Asia depth–damage curve (Table 2), and (iv) integrating over probabilities to EAD (Eq. 17).
# =========================

# Eq (11) GDP(x) = NTL(x) * GDP_PHL / sum_{x in C} NTL(x)
gdp_year <- 2018
gdp_usd <- tryCatch({
...
# This is the Huizinga et al. (2017) Asia depth–damage curve (Table 2) implemented via linear interpolation.
# Input: flood depth h (m). Output: damage fraction f(h) in [0,1].
...
# Eq (14) EAD integral
# We approximate expected annual damage (EAD) by integrating damages over exceedance probability p = 1/RP (Eq. 14)
risk_integral_ead <- function(rp, damage) {
  p <- 1 / rp
  ord <- order(p, decreasing=TRUE)
  p <- p[ord]; damage <- damage[ord]
  
  # Anchor at p=1 (very frequent, tiny events) with zero damage so EAD integrates on [0,1]
...
# =========================
# 9) CBA (PDF 4.5, Eq 15–17)
# We compute municipality-level cost–benefit metrics:
#  - NPV of avoided EAD over T years (discount rate r, growth g),
#  - restoration costs (USD/ha),
#  - benefit–cost ratios (BCR).
# =========================

# Eq (15) MangroveHa_i = sum MangroveFraction(x) * CellAreaHa(x)
...
# Eq (16) NPV_i = sum_t ΔEAD_i*(1+g)^(t-1)/(1+r)^t
# We compute the present value of a constant annual benefit stream over T years (Eq. 18–19).
# annual_benefit can be a vector (one value per municipality).
...
cba_tbl <- valuation_tbl |>
  dplyr::mutate(
    ...
    # Eq (17) BCR = NPV / (CostPerHa * MangroveHa)
    NPV_cost_low  = mangrove_ha * cost_low,
    NPV_cost_mid  = mangrove_ha * cost_mid,
    NPV_cost_high = mangrove_ha * cost_high,
    # Eq (17) BCR = NPV / Cost, defined only when costs exist
    BCR_low  = dplyr::if_else(mangrove_ha > 0, NPV_benefits_usd / NPV_cost_low,  NA_real_),
    BCR_mid  = dplyr::if_else(mangrove_ha > 0, NPV_benefits_usd / NPV_cost_mid,  NA_real_),
    BCR_high = dplyr::if_else(mangrove_ha > 0, NPV_benefits_usd / NPV_cost_high, NA_real_)
  )
...
# =========================
# 10) DISTRIBUTIONAL ANALYSIS (Section 4.6)
# We study how benefits are distributed by ranking municipalities by night-lights-per-capita (a rough income proxy).
# We then compute the share of total avoided EAD accruing to the bottom 40% group (paper distributional summary).
# =========================
...
# =========================
# 11) MAPS AND FIGURES
# We generate paper-ready figures (light and dark styles) and save them as PNGs with consistent resolution.
# =========================
...
# 11.1 COMMON HELPERS
# -------------------------
# We save tmap objects using the base PNG device (avoids Cairo dependencies).
# This keeps export behaviour consistent across Windows/macOS/Linux.
...
# -------------------------
# 11.3 MAP 1: FLOOD RP10 DARK (fig1)
# -------------------------

# ---- Build flood plot for RP10 (local + robust) ------------------------------
rp_focus_chr <- if (exists("rp_focus_chr")) rp_focus_chr else as.character(RP_FOCUS)

flood_raw <- scenario$no_mangroves[[rp_focus_chr]]
flood_raw <- terra::ifel(flood_raw <= 0, NA, flood_raw)

# thicken so thin coastal ribbons are visible (matches your dark-style outputs)
flood_plot_dark <- thicken_plot_raster(flood_raw, fact = 2, win = 3)

# ---- bins + palette (match your legend labels) -------------------------------
...
# ---- TMAP object (legend hidden; we draw our own) ----------------------------
p2_dark <- tm_shape(phl_outline) +
  tm_borders(lwd = 0.20, col = "white") +
  tm_shape(coastal_outline) +
  tm_borders(lwd = 0.15, col = "white") +
  tm_shape(flood_plot_dark) +
  tm_raster(
  ...
  tm_title(
    paste0("Coastal inundation depth (RP", RP_FOCUS, ", baseline)"),
    color = "white",
    size = 1.10
  ) +
  tm_layout(
...
# ---- Save with a manual legend row at the bottom -----------------------------
save_flood_rp10_dark <- function(
    tm_obj,
    filename = "outputs/map_flood_rp10_dark.png",
    legend_title = NULL,
    legend_labels,
    legend_cols,
    width = 2200,
  ...
  if (is.null(legend_title)) {
    legend_title <- paste0("Flood depth (m)  RP", RP_FOCUS)
  }
  
  dir.create(dirname(filename), showWarnings = FALSE, recursive = TRUE)
  
  # split canvas: top map + bottom legend strip
  top_frac <- 0.93
  map_h <- round(height * top_frac)
  leg_h <- height - map_h
  
  tmp_map <- tempfile(fileext = ".png")
  on.exit(unlink(tmp_map), add = TRUE)
  
  # 1) render map alone (no legend) to a temp PNG
  save_tmap_png(tm_obj, tmp_map, width = width, height = map_h, res = res)
  img <- png::readPNG(tmp_map)
  
  # 2) compose final PNG with bottom legend row
  grDevices::png(filename, width = width, height = height, res = res, bg = "black")
  grid::grid.newpage()
  grid::grid.rect(gp = grid::gpar(fill = "black", col = NA))
  
  lay <- grid::grid.layout(
    nrow = 2, ncol = 1,
    heights = grid::unit(c(map_h, leg_h), "null")
  )
  grid::pushViewport(grid::viewport(layout = lay))
  
  # row 1: map
  grid::pushViewport(grid::viewport(layout.pos.row = 1, layout.pos.col = 1))
  grid::grid.raster(img, x = 0.5, y = 0.5, width = 0.98, just = "center", interpolate = FALSE)
  grid::popViewport()
  ...
  x_pos <- seq(0.06, 0.92, length.out = n)
  box_w <- 0.020
  box_h <- 0.32
  txt_dx <- box_w + 0.010
...
save_flood_rp10_dark(
  tm_obj        = p2_dark,
  filename      = "outputs/map_flood_rp10_dark.png",
  legend_labels = legend_labels,
  legend_cols   = flood_cols_blue,
  width         = 2200,
  height        = 1600,
  res           = 300
)
...
# -------------------------
# 11.4 MAP 2: MULTI-RP FLOOD DARK (fig2)
# -------------------------
flood_cols_blue <- c(
  "#9fe8ff", "#6fddff", "#3ed0ff", "#14c2ff",
  "#00adf0", "#0088e0", "#0060c7"
...
# -------------------------
# 11.9 MAP 7: BCR LOW: show BCR only where local mangroves and local cost exist
# -------------------------
...
# -------------------------
# 11.10 MAP Extra: EAD COMPARE with common legend
# -------------------------
phl_map_ead <- phl_adm2_m |>
  dplyr::left_join(final_tbl, by = c("GID_2", "NAME_1", "NAME_2"))
...

{
  ...
  grDevices::png(
    "outputs/map_ead_compare_same_leg.png",
    width = final_width,
    height = final_height,
    res = 300,
    bg = "black"
...
# We run sensitivity checks for the attenuation parameters (alpha, L) and (where enabled) the discount rate r.
# =========================

# --- Table 3: Top 20 municipalities by BCR low---------------------------------------

pkgs_tbl <- c("dplyr", "readr", "knitr", "kableExtra", "scales", "stringr")
to_install <- pkgs_tbl[!pkgs_tbl %in% installed.packages()[, "Package"]]
if (length(to_install)) install.packages(to_install)
invisible(lapply(pkgs_tbl, library, character.only = TRUE))

# --- choose a small threshold so we don't get division-by-zero / meaningless BCR ---
HA_MIN <- 1  # 1 hectare minimum (adjust to 0.1 if you want to be less strict)

table3_pretty <- final_tbl |>
  dplyr::filter(!is.na(mangrove_ha), mangrove_ha >= HA_MIN) |>
  dplyr::arrange(dplyr::desc(BCR_low)) |>
  dplyr::slice_head(n = 20) |>
  dplyr::transmute(
    Province      = NAME_1,
    Municipality  = NAME_2,
    `Mangroves (ha)` = mangrove_ha,
    
    # scale money into USD millions for readability
    `Avoided EAD (USD m/yr)` = EAD_avoided_usd / 1e6,
    `NPV (USD m)`           = NPV_benefits_usd / 1e6,
  ...
  dplyr::mutate(
    Province = stringr::str_trunc(Province, 20),
    Municipality = stringr::str_trunc(Municipality, 24)
  )

# round and formatting for display/export
table3_pretty_fmt <- table3_pretty |>
  dplyr::mutate(
    `Mangroves (ha)` = round(`Mangroves (ha)`, 1),
    `Avoided EAD (USD m/yr)` = round(`Avoided EAD (USD m/yr)`, 2),
    `NPV (USD m)` = round(`NPV (USD m)`, 2),
    `BCR low`  = round(`BCR low`, 2),
    `BCR mid`  = round(`BCR mid`, 2),
    `BCR high` = round(`BCR high`, 2)
  )

# --- LaTeX table output (Overleaf-friendly) ---
tab_tex <- knitr::kable(
  table3_pretty_fmt,
  format = "latex",
  booktabs = TRUE,
  longtable = FALSE,
  caption = "Top 20 municipalities by benefit--cost ratio (low-cost scenario).",
  label = "tab:top20_bcr",
  align = "llrrrrrr"
) |>
  kableExtra::add_header_above(c(" " = 5, "Benefit--Cost Ratio" = 3)) |>
...
writeLines(tab_tex, "outputs/table3_top20_bcr_municipalities.tex")

# write as a CSV version for checking
readr::write_csv(table3_pretty, "outputs/table3_top20_bcr_municipalities.csv")

# --- Other tables: Helper: NATIONAL EAD total from depth maps
